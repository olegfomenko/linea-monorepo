...
type TestCases struct {
	Inputs  [][]field.Element `json:"inputs"`
	Entries []struct {
		Params struct {
		...
		Expected [][]field.Element `json:"expected"`
	} `json:"entries"`
}
...
func TestMulMod(t *testing.T) {
	...
	domain := fft.NewDomain(size).WithShift(shift)
	...
	// expected result
	expectedr := make([]field.Element, 4)
...
func TestLimbDecomposition(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			logTwoDegree: 4,
			logTwoBound:  4,
			vec:          []field.Element{field.One()},
		},
		{
			logTwoDegree: 4,
			logTwoBound:  4,
			vec:          []field.Element{field.NewElement(2)},
		},
		{
			logTwoDegree: 4,
			logTwoBound:  4,
			vec:          []field.Element{field.NewElement(1 << 32), field.NewElement(2), field.NewElement(1)},
		},
		{
			logTwoDegree: 4,
			logTwoBound:  16,
	...
	for i, testcase := range testcases {
		...
		t.Run(fmt.Sprintf("testcase-%v", i), func(t *testing.T) {
			...
			dummyBuffer := make([]field.Element, 192)
			...
			b := field.NewElement(1 << sis.LogTwoBound)
			numLimbsPerField := field.Bytes * 8 / sis.LogTwoBound
...
func makeKeyDeterministic(t *testing.T, sis *RSis, _seed int64) {
	...
	polyRand := func(seed field.Element, deg int) []field.Element {
		res := make([]field.Element, deg)
		for i := 0; i < deg; i++ {
			res[i].Square(&seed)
			seed.Set(&res[i])
		}
...
func estimateSisTheory(p sisParams) float64 {
	...
	r := totalTimePoly * field.Bits / p.logTwoBound / (1 << p.logTwoDegree)
	return float64(r)
}
...
func BenchmarkSIS(b *testing.B) {
	...
	inputs := make([]field.Element, nbInputs)
...
func benchmarkSIS(b *testing.B, input []field.Element, sparse bool, logTwoBound, logTwoDegree int, theoretical float64) {
	b.Helper()
...
func (r *RSis) Hash(v []field.Element) ([]field.Element, error) {
	...
	rlen := len(sum) / field.Bytes

	var result []field.Element
	var buf [field.Bytes]byte
	for i := 0; i < rlen; i++ {
		// read the next field element
		copy(buf[:], sum[i*field.Bytes:(i+1)*field.Bytes])

		newElement, err := fr.BigEndian.Element(&buf)
		if err != nil {
			return nil, err
		}
...
func TestLimbDecompositionFastPath(t *testing.T) {
	...
	for size := field.Bytes; size < 5*field.Bytes; size += field.Bytes {
		// Test the fast path of limbDecomposeBytes8_64
		buf := make([]byte, size)
		m := make([]field.Element, size)
		mValues := bitset.New(uint(size))
		n := make([]field.Element, size)
...
func TestUnrolledFFT(t *testing.T) {
	...
	k1 := make([]field.Element, size)
	for i := 0; i < size; i++ {
		k1[i].SetRandom()
	}
	k2 := make([]field.Element, size)
	copy(k2, k1)
